...
const ProductForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!resp.ok) {
      // If we get an error, send the error json and the empty fields so we can highlight them
      setErr(json.error);
      setEmptyFields(json.emptyFields);
    }
    if (resp.ok) {
      // If we successfully add the product, reset fields, log it, and update the inventory context
      setName("");
      setQuantity("");
      setErr(null);
      setEmptyFields([]);
