...
const Shipping = () => {
  ...
  const fetchOrders = async () => {
    try {
      setOrders(
        (await axios.get("/api/staff/orders/")).data.filter(
  ...
  const printFile = (rawHTML, fileName, fileType) => {
    ...
    const url = window.URL.createObjectURL(
      new Blob([rawHTML], { type: fileType })
    );
  ...
  const printInvoice = (order) => {
    ...
    var invoice = `<h1>Invoice</h1>
            <p><strong>To:</strong> ${order.shippingAddress.name}<br/>${
      order.shippingAddress.street
    }<br/>${order.shippingAddress.city}, ${order.shippingAddress.state}, ${
      order.shippingAddress.zip
    }</p>
            <p><strong>Date:</strong> ${order.updatedAt.substring(
              0,
              order.updatedAt.indexOf("T")
            )}
            <p>Parts:</p><table><tr><th>Part Number</th><th>Quantity</th></tr>`;
    for (let product of order.parts) {
      invoice += `<tr><td>${product.partNumber}</td><td>${product.quantity}</td></tr>`;
    }
    invoice += `</table><br/><table><tr><td>Subtotal:</td><td>${formatter.format(
      order.subtotal.$numberDecimal
    )}</td></tr><tr><td>Shipping:</td><td>${formatter.format(
      order.shippingAndHandling.$numberDecimal
    )}</td></tr><tr><td>Total:</td><td>${formatter.format(
        ...
        {orders &&
          orders.map((order) => (
            <div className="product-details">
              ...
              <br />
              <button onClick={() => printPackingList(order)}>
                Print packing list
              </button>
              ...
              <button onClick={() => confirmShipment(order)}>
                Confirm shipment
              </button>
            </div>
          ))}
